...
class ResultsByCategory extends Component {
  ...
  fetchResultsByComCategory = () => {
    ...
    !this.state.selectedCategory
      ? (category = this.props.competitiveCategory[0]._id)
      : (category = this.state.selectedCategory);
    this.props.onFetchResultsByComCategory(category, this.props.token);
  };
  writeFinalResultsHandler = () => {
    const catId =
      this.state.selectedCategory || this.props.competitiveCategory[0]._id;
    const index = this.props.competitiveCategory.findIndex(
      (i) => i._id === catId
    );
    ...
    return (
      <ElementWrapper wrapperType="ElementWrapper">
        ...
        <ElementWrapper wrapperType="FullWidthWrapper">
          ...
          <p>
            Stav výsledkov:{" "}
            {this.props.isFinalResultWrite
              ? "Finálne výsledky - zapísané"
              : "Priebežné výsledky - nezapísané"}
          </p>
          <div className={classes.HeaderCategoryChoose}>
            <label>Súťažná kategória vína</label>
            <select onChange={this.getCategoryHandler}>
            ...
            <DownloadFile
              ...
              isComplete={isTrueCheck(
                this.props.competitiveCategory,
                "isFinalResultWrite"
              )}
            >
            ...
            >
              Zapísať výsledky skupiny
            </Button>
          </div>
          {this.props.loading ? (
            <Spinner />
          ) : (
            <div className={classes.ResultsByCategoty_table}>
              <CategoryTable
...
const mapDispatchToProps = (dispatch) => {
  return {
    onFetchCompetitiveCategory: (token) =>
      dispatch(action.fetchCompetitiveCategory(token)),
    onFetchResultsByComCategory: (categoryId, token) =>
      dispatch(action.fetchWineResultsByComCategory(categoryId, token)),
    onWriteFinalResults: (catId, index, token) =>
      dispatch(action.writeFinalResults(catId, index, token)),
  };
};
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(ResultsByCategory));
