...
class ResultsByDegustator extends Component {
  ...
  fetchResultByDegustator = () => {
    let selectedDeg;
    !this.state.selectedDeg
      ? (selectedDeg = this.props.degustators[0]._id)
      : (selectedDeg = this.state.selectedDeg);
      ...
      <ElementWrapper wrapperType="ElementWrapper">
        ...
        <ElementWrapper wrapperType="FullWidthWrapper">
          ...
          {this.props.loading ? (
            ...
            <div className={classes.ResultsByDegustator_table}>
...
const mapDispatchToProps = (dispatch) => {
  return {
    onfetchResultsByDeg: (degId, token) =>
      dispatch(action.fetchResultsByDeg(degId, token)),
    onFetchListOfDegustator: (token) =>
      dispatch(action.fetchListOfDegustators(token)),
  };
};
