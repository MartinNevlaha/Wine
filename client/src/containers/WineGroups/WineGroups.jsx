    ...
    let content = (
      <React.Fragment>
        ...
        <Popup
          ...
          show={this.props.error || this.props.isSucces || this.state.error}
          message={message}
        />
      </React.Fragment>
    );
...
const mapDispatchToProps = (dispatch) => {
  return {
    onFetchWineEditGroups: (token) =>
      dispatch(action.fetchEditWineGroups(token)),
    onWineGroupChange: (choosenWineData, groupDbId, groupName) =>
      dispatch(action.wineGroupChanged(choosenWineData, groupDbId, groupName)),
    onSortWineGroups: (sortByProp) =>
      dispatch(action.sortWineGroupsBy(sortByProp)),
    onSaveWineGroups: (wineGroupsData, token) =>
      dispatch(action.saveWineGroups(wineGroupsData, token)),
  };
};
