...
exports.getFinalResultsByCategory = async (req, res, next) => {
  ...
  try {
    ...
    const wines = await Wine.find({ competitiveCategoryId: categoryId })
      .sort({ finalResult: -1 })
      .lean()
...
exports.getFinalResultsByWineId = async (req, res, next) => {
  ...
  try {
    const results = await Result.find({ wineDbId: wineId }).populate(
      populateQuery
    );
    if (!results) {
...
exports.getFinalResultsByGroup = async (req, res, next) => {
  ...
  try {
    const results = await Result.find({ groupId: groupId })
      .populate(populateQueryDeg)
      .populate(populateQueryWine);
    if (!results) {
...
exports.getFinalResultsByDeg = async (req, res, next) => {
  ...
  try {
    const results = await Result.find({ degId: degId })
      .populate(populateQueryDeg)
      .populate(populateQueryWine);
    if (!results) {
      const error = new Error("Nemôžem načítať výsledky");
...
exports.getWineCompetitionCategory = async (req, res, next) => {
  try {
    ...
    const results = await Wine.find({
      competitiveCategoryId: competitiveCategory[0]._id,
    })
      .sort({ finalResult: -1 })
      .lean()
      .populate("group");
...
exports.getDegGroups = async (req, res, next) => {
  ...
  try {
    ...
    const results = await Result.find({ groupId: groups[0]._id })
      .populate(populateQueryDeg)
      .populate(populateQueryWine);
    if (!results) {
      const error = new Error("Nemôžem načítať výsledky");
...
exports.getListOfDegustators = async (req, res, next) => {
  ...
  try {
    ...
    const results = await Result.find({ degId: degustators[0]._id })
      .populate(populateQueryDeg)
      .populate(populateQueryWine);
    if (!results) {
      const error = new Error("Nemôžem načítať výsledky");
...
exports.exportResults = async (req, res, next) => {
  ...
  const populateQueryWine = {
    ...
    model: "Wine",
    select:
      "id name clasification color character _id vintage producer competitiveCategory",
  };
  try {
    const categories = await CompetitiveCategory.find({});
    const groups = await Group.find({}, "_id groupName");
    const completeResults = await Result.find({})
      .populate("wineDbId")
      .populate("degId");
    ...
    for await (let cat of categories) {
      ...
      const results = await Wine.find({ competitiveCategoryId: cat._id })
        .sort({ finalResult: -1 })
        .lean()
        .populate(populateQueryCat);
      if (!results) {
    ...
    for await (let group of groups) {
      let worksheet = workbook.addWorksheet(
        `Degustacna_skupina_${group.groupName}`
      );
      worksheet.columns = [
        { header: "Číslo Vína", key: "id" },
        { header: "Názov", key: "name" },
    ...
    worksheet.columns = [
      ...
      {
        header: "Chuť harmonická perzistencia",
        key: "tasteHarmonicPersistence",
      },
...
exports.generatePdf = async (req, res, next) => {
  ...
  try {
    ...
    const pdf = await generatePdf({
      data: data,
      templateName: "cert-template.html",
      isLandscape: true,
    });
    if (!pdf) {
...
exports.writeFinalResults = async (req, res, next) => {
  const categoryId = req.params.categoryId;
  try {
    const wines = await Wine.find({ competitiveCategoryId: categoryId }, "_id")
      .sort({ finalResult: -1 })
      .lean();
    if (!wines) {
      const error = new Error("Nemôžem načítať výsledky pre túto kategóriu");
